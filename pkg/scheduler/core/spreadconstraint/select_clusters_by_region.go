...
func selectBestClustersByRegion(spreadConstraintMap map[policyv1alpha1.SpreadFieldValue]policyv1alpha1.SpreadConstraint,
	...
	var selectClusters []ClusterDetailInfo
	...
	restCnt := needCnt - len(selectClusters)
	if restCnt > 0 {
		...
		for i := 0; i < restCnt; i++ {
			selectClusters = append(selectClusters, candidateClusters[i])
		}
	}

	return selectClusters, nil
}
